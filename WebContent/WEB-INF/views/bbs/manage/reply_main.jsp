...
<body>
	...
	<script>
		$("#addReply").submit(function(){
			$.ajax({
	    		url:"${pageContext.request.contextPath}/bbs/addReply",
	    		type:"POST", 
	    		data:$("#addReply").serialize(),
	    		dataType:"json",
	    		success:function(data){ 
		    		if(data.resultCode=="0"){
		    			window.location.href="${pageContext.request.contextPath}/bbs/toViewReply?postId=${post.id}"
		    		}else{ 
		...
		$(function(){
			...
			$("#next").click(function(){
				currPage=++currPage;
				window.location.href="${pageContext.request.contextPath }/bbs/toViewReply?postId="+topicId+"&page="+currPage;
			})
			$("#pre").click(function(){
				currPage=--currPage;
				window.location.href="${pageContext.request.contextPath }/bbs/toViewReply?postId="+topicId+"&page="+currPage;
